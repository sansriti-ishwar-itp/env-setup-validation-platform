...
![Environment Setup Validation Platform architecture](architecture.svg)
...
The ADK FastAPI app from `google.adk.cli.fast_api.get_fast_api_app` is **mounted at `/adk`**, so you get the endpoints documented in the [ADK REST API Reference](https://google.github.io/adk-docs/api-reference/rest/), including:
...
Every agent run writes a separate runtime log under [`logs/`](logs/) using the file name `agent-session-<run_id>.jsonl`. The JSONL entries include run creation, status changes, ADK session lifecycle events, raw ADK stream events, tool events, operator interventions, approvals, retries, cancellations, failures, and apply results.

These logs are generated by the backend while the agent runs.
...
- **Repo-local session logs** mirror each run into `logs/agent-session-<run_id>.jsonl` so ADK/tool activity can be reviewed outside SQLite.

## Assignment checklist

- [ ] Meaningful **git commits** during development (not one giant commit).
- [x] Store per-session **agent runtime logs** under [`logs/`](logs/) as separate generated JSONL files.
- [x] Include the architecture diagram in [`architecture.svg`](architecture.svg).

## What we would build next

Given more time, the next priority would be moving from direct branch commits to a **merge request workflow**. The platform already keeps writes behind human approval, but opening a merge request would add code review, CI checks, discussion, and rollback-friendly collaboration before changes land on the target branch.

The second priority would be deeper dependency and security analysis. For Python projects, integrating `pip-compile`, `uv lock`, or another lockfile-aware workflow would make dependency pinning recommendations safer. Adding focused `semgrep` or `bandit` checks would catch common security-adjacent setup issues without claiming full vulnerability coverage.

Finally, expand observability around run duration, tool failures, and apply outcomes.